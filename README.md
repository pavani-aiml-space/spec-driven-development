...
specs/     Persisted specs / test plans - the durable "current truth" artifacts
templates/ Worked examples for skills that have one
```
...
Persisted spec artifacts - currently holds QA test plans (e.g. `get-started.plan.md`). This is the natural home for `concept`/`design` outputs that should stay around as checkable "current truth" once a unit of work reaches `closeout`, rather than living only in chat history.

## Templates

Browsable copies of worked examples, one per skill that has one - see [templates/](templates/README.md).
